...
export default function Harada({ loaderData }: Route.ComponentProps) {
	...
	const renderCell = (
		...
		const needsExtraTopGap = rowIndex % 3 === 0 && rowIndex !== 0;
		const needsExtraLeftGap = colIndex % 3 === 0 && colIndex !== 0;
		const quadrantGapClass =
			`${needsExtraTopGap ? 'mt-1.5 sm:mt-3 md:mt-3' : ''} ${needsExtraLeftGap ? 'ml-1.5 sm:ml-3 md:ml-3' : ''}`.trim();

		if (!cellToRender) {
			return (
				<div
					aria-hidden
					className={`h-full w-full ${quadrantGapClass}`}
		...
		return (
			<div
				className={`group relative h-full w-full ${quadrantGapClass}`}
				key={`${cellToRender.row}-${cellToRender.col}`}
			>
				<button
					aria-describedby={tooltipId}
					aria-label={cellToRender.text}
					className={`surface gap-1.5 flex h-full w-full flex-col items-center px-2 pb-2 text-center text-[11px] leading-snug transition-colors duration-150 sm:text-xs md:text-sm ${styles.cell}`}
					type="button"
				>
					<div className="flex w-full justify-start">
						<span className="pointer-events-none px-2 py-0.5 text-[10px] uppercase tracking-wide">
							{directionLabel}
						</span>
					</div>
				...
				<div
					className={`pointer-events-none absolute z-30 w-64 px-3 py-2 text-left text-xs opacity-0 shadow-2xl transition-all duration-150 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100 ${horizontalTooltipClass} ${verticalTooltipClass}`}
				>
					<div className="mb-1 flex items-center gap-2">
						<span
							className={`px-2 py-0.5 text-[10px] uppercase tracking-wide ${styles.badge}`}
						>
							{styles.label}
						</span>
	...
	return (
		<main
			className="mx-auto flex max-w-7xl flex-col gap-10 px-4 py-10"
			id="main-content"
			tabIndex={-1}
		>
			<header className="flex flex-col gap-4 pb-6">
				<div className="flex flex-wrap items-center justify-between gap-3">
					<div>
						<h1 className="text-3xl tracking-tight">{t('harada.title')}</h1>
						<p className="text-sm">
				...
				<div className="grid gap-3 p-4">
					<div className="flex flex-wrap items-center gap-2 text-sm">
						<span className=" px-2 py-1 text-[11px] uppercase tracking-wide">
							{t('harada.metaLabel')}
						</span>
			...
			<section className="space-y-4">
				<div className="flex items-center justify-between">
					<h2>{t('harada.gridTitle')}</h2>
					<p>{t('harada.gridHint')}</p>
				</div>
				<div className="p-3">
					<div className="grid aspect-square w-full grid-cols-9 gap-0.5 sm:gap-1 md:gap-1">
			...
			<section className="grid gap-4 p-4">
				<h3 className="text-base">{t('harada.themesTitle')}</h3>
				<ol className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
					{themes.map((theme: string, index: number) => (
						<li
							...
							key={theme}
						>
							<span className="mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center text-xs">
								{index + 1}
							</span>
